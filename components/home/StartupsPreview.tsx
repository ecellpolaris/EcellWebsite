import React from "react";
import Link from "next/link";
import { STARTUPS } from "@/lib/data/startups";
import { Badge } from "@/components/ui/Badge";
import { ArrowRight, ExternalLink } from "lucide-react";

export const StartupsPreview: React.FC = () => {
  const featured = STARTUPS.filter((s) => s.featured).slice(0, 3);

  return (
    <section className="py-20 border-b border-paper-border bg-paper-sunken">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-14">
          <div className="space-y-3 max-w-xl">
            <span className="micro-label text-ink-muted text-xs font-mono tracking-widest block">
              CAMPUS PROOF OF WORK
            </span>
            <h2 className="font-display font-extrabold text-3xl sm:text-4xl text-ink tracking-tight">
              Startups born in these rooms.
            </h2>
            <p className="text-sm sm:text-base text-ink-muted">
              Not imaginary case studies. Real code, real customers, and early revenue generated by undergraduate Polarisians.
            </p>
          </div>

          <Link
            href="/startups"
            className="font-mono text-xs font-bold text-ink underline underline-offset-4 hover:text-ink-muted flex items-center gap-1 shrink-0"
          >
            <span>View Full Portfolio ({STARTUPS.length} Startups)</span>
            <ArrowRight className="w-3.5 h-3.5" />
          </Link>
        </div>

        {/* 3 Featured Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          {featured.map((startup) => (
            <div
              key={startup.slug}
              className="bg-paper-card border border-paper-border rounded-[4px] p-6 shadow-card flex flex-col justify-between hover:border-ink/50 transition-all space-y-6"
            >
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-paper-border pb-3">
                  <span className="font-mono text-xs uppercase font-bold text-ink-muted">
                    {startup.sector}
                  </span>
                  <Badge
                    variant={
                      startup.stage === "Funded"
                        ? "lime"
                        : startup.stage === "Incorporated"
                        ? "teal"
                        : "muted"
                    }
                    size="xs"
                  >
                    {startup.stage}
                  </Badge>
                </div>

                <div className="space-y-1">
                  <h3 className="font-display font-extrabold text-2xl text-ink flex items-center justify-between">
                    <span>{startup.name}</span>
                    {startup.websiteUrl && (
                      <span className="text-ink-muted hover:text-ink">
                        <ExternalLink className="w-4 h-4" />
                      </span>
                    )}
                  </h3>
                  <p className="text-xs font-mono text-ink-muted">
                    Founders: {startup.founders.map((f) => `${f.name} (${f.year})`).join(", ")}
                  </p>
                </div>

                <p className="text-sm text-ink leading-relaxed">
                  {startup.oneLiner}
                </p>

                {/* What E Cell Did */}
                <div className="p-3 bg-paper-sunken border border-paper-border rounded-[3px] text-xs">
                  <span className="font-mono font-bold text-ink block mb-0.5">
                    HOW E CELL HELPED:
                  </span>
                  <p className="text-ink-muted leading-relaxed">
                    {startup.whatEcellDid}
                  </p>
                </div>
              </div>

              {/* Metrics Bar */}
              <div className="pt-4 border-t border-paper-border grid grid-cols-2 gap-2 text-xs font-mono">
                {startup.metrics.slice(0, 2).map((m, idx) => (
                  <div key={idx}>
                    <span className="text-ink-muted block text-[10px] uppercase">
                      {m.label}
                    </span>
                    <span className="font-bold text-ink text-xs sm:text-sm">
                      {m.value}
                    </span>
                  </div>
                ))}
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
};
